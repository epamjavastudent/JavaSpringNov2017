...
https://ru.wikipedia.org/wiki/%D0%A3%D1%80%D0%BE%D0%B2%D0%B5%D0%BD%D1%8C_%D0%B8%D0%B7%D0%BE%D0%BB%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D0%BE%D1%81%D1%82%D0%B8_%D1%82%D1%80%D0%B0%D0%BD%D0%B7%D0%B0%D0%BA%D1%86%D0%B8%D0%B9#%D0%9D%D0%B5%D0%BF%D0%BE%D0%B2%D1%82%D0%BE%D1%80%D1%8F%D1%8E%D1%89%D0%B5%D0%B5%D1%81%D1%8F_%D1%87%D1%82%D0%B5%D0%BD%D0%B8%D0%B5
 ...
 НОВЫЙ ГОД
   ...
   JDBC - последняя лекция.
   
   Metadata 
   - метаданные - данные о структуре БД и её составных частей
           
ResultSetMetaData (дает инфу о числе колонок результирующем наборе, метки и имя колонки, таблица - источник данных для колонки, тип даных колонки),
результат SQL-запроса формируется в форме таблицы.
(!) не забыть закрыть.

DataBaseMetaData (метаинформация о БД в  целом)
   Connection.getMetaData();

Debug возможен в однопоточном приложении. 
Для многопоточных - используются LOG'и.


   язык Ceylon компилируется в Java.
    
   https://ru.wikipedia.org/wiki/MD5


Автосгенерированные ключи:
    НЕпозволяет работать с БД асинхронно.
    
   UUID (universally unique identifier)  
   - алгоритм авто-генерации ключа в форме строки, c крайне низкиой вароятностью совпадения 
   (совпадение возможно после >2^66).
...
решает задачу передачи доступа к одному объекту между несколькими приложениями в кластере серверов.
Сейчас это используется в ConnectionPool. В качестве ID объекту присваивается уникальная строка, по которой 
осуществляется к нему доступ.

JNDI предназначен для маппинга имени к объектам в распределенной системе.



  Все админки являются обертками для конфигов. В Tomcat нет админки.
  


Spring Data JDBC.

модель JDBC более гибкая, прозрачная и производительная, чем ORM (Object-Relational Mapping).

Hibernate native SQL queries.

JOOQ самостоятельно генерит JDBC запросы.
Hibernate лучше JDBC, а JOOQ лучше Hibernate'а.

JOOQ позволяет строить SQL прямо в Java-коде.  
Jooq включает в себя генератор кода, который парсит структуру вашей базы данных и создает необходимые Java-классы.
...
Liquibase и FlyWeight позволяет управлять миграциями БД, т.е. аналог отката к прерыдущей версии в GIT, но уже для БД.


rowMapper - описывает один запрос к базе, а затем Spring применяет его ко всем элементам базы самостоятельно.
...
Основные сущности:
   ...
   Hibernate:
    ...
    Lesson 2018-01-15. Hibernate
 
Посмотреть доклад (!) по Spring Data  https://www.youtube.com/watch?v=nwM7A4TwU3M

id лучше делать Long (может иметь NULL).


Языки подобные SQL, в которых вместо сущностей - имена полей.
JPQL (в JPA)
HQL (в Hibernate)

    
    
В Hibernate все поля автоматически являются column.
@Column (name = "") следует указывать, если имя поля не совпадает с именем в БД.


SELECT * мы не пишем, он рабоатет по умолчанию,  SELECT используют по специфицированному запросу. 


Основная проблема Hibernate - 3-х уровневое кэширование.
Данные могут быть в cache, но они никуда не отправлялись.
